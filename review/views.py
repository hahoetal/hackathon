...
def review(request, data_id): # 음식에 대한 상세 정보를 불러와야 하므로 url을 통해 data_id를 받아온다.(url도 고쳐주기)
    ...
    # method가 GET 방식인 경우.
        ...
        reviews_list = Userreview.objects.filter(menu__contains=menu_name) # Usearreview 클래스로 만든 모든 객체를 가져되, 가져온 Data객체와 메뉴명이 동일한 경우에 가져오고,
        paginator = Paginator(reviews_list, 3) # Paginator를 이용해서 객체들을 세 개씩 분할하고,
        page = request.GET.get('page') # request된 페이지를 paga에 담고,
